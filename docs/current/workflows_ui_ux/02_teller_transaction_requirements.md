...
**Purpose:** Record miscellaneous income (fees, sales, etc.) with configurable receipt types. Supports payment by cash, account debit, and/or checks.

**Required (server):**

- `misc_receipt_type_id` — receipt type (or `income_account_reference` when type supplies default)
- `income_account_reference` — account to credit (required if type does not supply default)
- `amount_cents` > 0 — total receipt amount
- `entries` — balanced legs (debits = payment sources: cash + account + checks; credit = income account)

**Conditional:**

- `memo` — required when misc receipt type has `memo_required?` set
- `cash_account_reference` — required when `misc_cash_cents` > 0 (drawer)
- `primary_account_reference` — required when `misc_account_cents` > 0
- Payment components must sum to amount: `misc_cash_cents + misc_account_cents + check_total_cents == amount_cents`

**Optional (stored in metadata):**

- `unit_amount_cents`, `quantity`, `memo`, `check_items[]` (routing, account, number, amount, etc.)

**Posting:** Debit cash account (physical cash received), primary account (account-funded portion), and/or check account refs; credit income account. Total debits = total credits = amount.
...
- **Form exposes:** Misc receipt type select, unit amount, quantity, memo; payment section (cash, account, checks) with amounts that must sum to total.
