  ...
  class AccountOwnersController < BaseController
    ...
    def create
      ...
      if @account_owner.save
        redirect_to teller_account_path(@account), notice: "Owner added."
      else
        redirect_to teller_account_path(@account), alert: @account_owner.errors.full_messages.to_sentence
      end
    end

    def destroy
      if @account_owner.destroy
        redirect_to teller_account_path(@account), notice: "Owner removed."
      else
        redirect_to teller_account_path(@account), alert: @account_owner.errors.full_messages.to_sentence
      end
    end
    ...
    def update
      if @account_owner.update(account_owner_update_params)
        redirect_to teller_account_path(@account), notice: "Primary owner updated."
      else
        redirect_to teller_account_path(@account), alert: @account_owner.errors.full_messages.to_sentence
      end
    end
    ...
    private
      ...
      def ensure_authorized
        authorize([ :teller, @account ], :update?, policy_class: Teller::AccountPolicy)
      end
